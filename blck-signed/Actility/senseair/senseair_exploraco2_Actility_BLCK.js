/**
 * Actility (ThingPark) payload decoder for Senseair ExploraCO2 (CO2/Temp/Humidity/PM Sensor)
 * @author       BLCK-IoT.com
 * @license      MIT
 * @version      1.0.0
 * @param  {number}     fPort
 ...
 * Device   : Senseair ExploraCO2 (CO2/Temp/Humidity/PM Sensor)
...
function _vendorDecodeUplink(input) {
    ...
    if (port===2) {
        ...
        var temp = (bytes[2] & 0xF0) >> 4;
        temp = temp | bytes[0] << 4;
        decoded.temperature = (temp - 800) / 10.0;

        var humidity = bytes[2] & 0x0F;
        humidity = humidity | bytes[1]<<4;
        decoded.humidity = (humidity -250)/10.0;
        ...
        if (bytes.length === 5) {
            ...
            if (decoded.co2 <= 600) {
                ...
                decoded.air_quality = "Excellent";

            } else if (decoded.co2 <= 1000) {

                decoded.air_quality = "Fair";

            } else if (decoded.co2 <= 1500) {

                decoded.air_quality = "Mediocre";

            } else if (decoded.co2 <= 2500) {

                decoded.air_quality = "Bad";

            } else {

                decoded.air_quality = "Extremely Bad";
            }

        } else if (bytes.length === 9) {

            decoded.PM10 = bytes[3]<<8 | bytes[4];
            decoded.PM25 = bytes[5]<<8 | bytes[6];
            decoded.PM1  = bytes[7]<<8 | bytes[8];
        }
    }
