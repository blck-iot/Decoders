/**
 * Actility (ThingPark) payload decoder for LDDS75 - Distance Sensor
 * @author       BLCK-IoT.com
 * @license      MIT
 * @version      1.0.0
 * @param  {number}     fPort
...
// BLCK correction (2026-08-14, confirmed against docs.dragino.com's LDDS75
// page + verified as the identical bug pattern already fixed in LDDS20):
// the original vendor code assigned the battery-derived `value` (bytes[0-1])
// to `data.distance`, while the REAL distance reading (bytes[2-3]) was
// stored under the unused field name `data.value`. Fixed to assign each
// field to its own correct byte range; the doc's own text ("all value lower
// than 280mm will be set to 0x0014(20mm) which means invalid") confirms the
// `< 20` threshold check itself was already correct, just applied to the
// wrong variable.
