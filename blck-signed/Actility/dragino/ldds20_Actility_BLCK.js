/**
 * Actility (ThingPark) payload decoder for LDDS20 - Liquid Level Sensor
 * @author       BLCK-IoT.com
 * @license      MIT
 * @version      1.0.0
 * @param  {number}     fPort
...
// BLCK correction (2026-08-14, verified directly against this file's own
// internal comments -- no external doc needed): the original vendor code
// assigned the battery-derived `value` (bytes[0-1], the SAME quantity used
// two lines above for `batV`) to `data.distance`, while the REAL distance
// reading (bytes[2-3]) was stored under the unrelated/orphaned field name
// `data.value` and never used. `data.batV` was computed but never included
// in the output at all. Flagged by an independent LLM documentation-review
// pass, confirmed by inspection (the code's own "//distance,units:mm"
// comment sits directly beside a line that clearly assigns the
// battery-voltage-related value, contradicting itself) -- fixed to assign
// each field to its own correct, real byte range.
  ...
  switch (input.fPort) {
    case 2:
      if (len == 5) {
        ...
        if (distance < 20) data.distance = 'Invalid Reading';
