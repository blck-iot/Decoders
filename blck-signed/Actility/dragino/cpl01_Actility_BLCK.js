/**
 * Actility (ThingPark) payload decoder for CPL01 - Dry Contact Sensor
 * @author       BLCK-IoT.com
 * @license      MIT
 * @version      1.0.0
 * @param  {number}     fPort
 ...
 * Device   : CPL01 - Dry Contact Sensor
...
// BLCK correction (2026-08-14): official Dragino CPL01 doc
// (docs.dragino.com/docs/LoRaWAN-End-Node/door-water-leak-contact-sensors/cpl01/)
// states "Contact Status: 0: Open, 1: Close" -- the vendor code had this
// bit polarity inverted (bit=1 mapped to OPEN, not CLOSE). Fixed to match
// the documented mapping; verified against a grounded fetch of the real
// manufacturer page, not training-data recall.
function datalog(i, bytes) {
  var ff = (bytes[0 + i] & 0xfc) >> 2;
  var aa = bytes[0 + i] & 0x02 ? 'TRUE' : 'FALSE';
  var bb = bytes[0 + i] & 0x01 ? 'CLOSE' : 'OPEN';
    ...
    case 2:
      {
        ...
        data.PIN_STATUS = bytes[0] & 0x01 ? 'CONNECT' : 'DISCONNECT';  // BLCK correction 2026-08-14 -- polarity was inverted vs official doc (0=Open/Disconnect, 1=Close/Connect)
